...
void strategy(Passenger &pass, string start, my_vector<string> &passcity, my_list<Path> &tmp_route,  double &mindanger, double &sumdanger, my_vector<double> &increment, int arrivetime, int limit, int choice) {//最少风险策略
	...
	if (sumdanger > mindanger) {//如果风险已经超出最小的，舍去
		...
		tmp_route.pop_back();
		sumdanger -= increment.back();//把刚才删除路径的的风险删去
		increment.pop_back();
		return;
	}
	if (choice && arrivetime > limit) {//如果是限时最小风险策略并且超出限定时间,舍去
		tmp_route.pop_back();//把最新加的路径点删除
		tmp_route.pop_back();
		sumdanger -= increment.back();
		increment.pop_back();//把刚才删除路径的的风险删去
		return;
	}
	while (begin_passcity != end_passcity) {
		if (begin_passcity != nullptr) {
			if (start == (*begin_passcity)) {//环路出现,舍去
				tmp_route.pop_back();
				tmp_route.pop_back();
				sumdanger -= increment.back();
				increment.pop_back();
	...
	if (start == pass.destination) {//到达终点，且风险比旧的最小风险低
		...
		if(fabs(sumdanger-mindanger)<1e-8)//如果新的路径风险于旧的相同，就添加
		...
		auto begin_city = city.begin(), end_city = city.end();//寻找出发城市的下标
		int i = 0;
		...
		while (begin_enable != end_enable) {
			...
			else if (arrivetime <= (*begin_enable).s_time) {//出发时间在当天
		...
		if (tmp_route.begin() != tmp_route.end()) {//路线不为空时,该条线路不可用，要删去路径点
			tmp_route.pop_back();
			tmp_route.pop_back();
...
void makepath() {
	...
	while (begin_pass != end_pass) {//变量设在里面，这样生命周期等于规划完一个乘客的路线的时间
		my_list<Path> tmp_route;//存放暂时的路径
